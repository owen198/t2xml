...
| Variable | Current default | Question | Notes |
| --- | --- | --- | --- |
| Train/dev/test split ratio (`split_for_file()` in `preprocess.py`) | 90/5/5 by `md5(filename) % 100`, assigned **per source XML file** (not per pair), so all snippets from one document land in the same split. Actual outcome on 496 files: 437 train (88.1%) / 34 dev (6.9%) / **25 test (5.0%)** → 750 test corpus docs, 720 test queries | Is a 5% test slice enough to evaluate on, given the corpus is only 496 documents to begin with? | - File-level (not pair-level) assignment is the right call and should stay — pair-level would put a `<dmodule>` in train and its own `<dmIdent>` child in test, leaking the answer outright given the nesting overlap documented below.<br>- But 90/5/5 is a large-corpus ratio applied to a small corpus: **the test split is 25 files / 750 docs**. SANTA's smallest benchmark (ruby) ranks against 4,360 docs; go against 28,120 — t2xml's pool is ~6× smaller than SANTA's smallest, so a random baseline scores much higher here and MRR differences carry less signal.<br>- Also unbalanced in practice: dev drew 34 files (6.9%) vs. test's 25 (5.0%), because md5-bucketing 496 items doesn't converge to the nominal ratio. The evaluation split is the *smaller* of the two, which is backwards — dev only needs to rank checkpoints, test carries the reported number.<br>- Compounding: the folders differ wildly in size/style (BIKE dominates), and hash assignment ignores that, so which folders land in test is arbitrary — 25 files is far too few to be representative across BIKE/FOSSIG/spec-sample/s1kd-tools.<br>- Candidate directions: (a) shift to something like 80/10/10 for a ~1,500-doc test pool; (b) stratify assignment by folder so each split covers all four sources proportionally; (c) k-fold cross-validation over the 496 files, reporting mean±std — the most defensible option at this corpus size, and it removes the "which 25 files happened to land in test" lottery entirely.<br>- Interacts with the two rows below: a larger/differently-composed test split changes the nesting-overlap and lexical-leakage picture too, so these should be decided together, not one at a time. |
| Query/doc overlap from deriving the retrieval benchmark out of `sda_pairs.*.jsonl` | `evaluate_xml.sh` on the test split scored MRR 0.88 — content-hash dedup (Pass 1) only collapses byte-identical `structured` strings into one corpus entry | Is that 0.88 inflated by overlap the dedup step doesn't catch, rather than reflecting genuine retrieval difficulty? | - **Confirmed**: nested elements (e.g. `dmodule` → `dmAddress` → `dmCode`) each become their own corpus doc, and since each is a text substring of its parent, exact-hash dedup doesn't catch them. Their queries are near-paraphrase, sharing the same rare anchors (dates, issue numbers, `dmCode` values) — a retriever can win on shared rare tokens within a file's family instead of real structural matching, likely inflating MRR.<br>- Fix candidates: **(A) keep only the deepest/most specific node per chain** (favored), (B) keep only the shallowest whole-subtree node, (C) exclude same-chain docs from each other's candidate pool — **not recommended**, makes the query artificially easier and inflates MRR further. |
