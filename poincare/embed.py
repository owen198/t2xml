...
# Fix for macOS multiprocessing with Cython classes
...
def main():
    ...
    final_msg = best_lmsg if best_lmsg is not None else (lmsg if 'lmsg' in locals() else None)
    if final_msg:
        eval_file = opt.checkpoint + '.eval.txt'
        with open(eval_file, 'w') as f:
            f.write('=== FINAL EVALUATION RESULTS ===\n')
            f.write(f'Checkpoint: {opt.checkpoint}\n')
            f.write(f'Dataset: {opt.dset}\n')
            f.write(f'Manifold: {opt.manifold}\n')
            f.write(f'Model: {opt.model}\n')
            f.write(f'Dimension: {opt.dim}\n')
            f.write(f'Learning Rate: {opt.lr}\n')
            f.write(f'Epochs: {opt.epochs}\n')
            f.write(f'\n--- Results ---\n')
            f.write(f'Best Epoch: {final_msg["epoch"]}\n')
            f.write(f'Elapsed Time: {final_msg["elapsed"]:.2f}s\n')
            f.write(f'Final Loss: {final_msg["loss"]:.6f}\n')
            
            if opt.eval == 'reconstruction':
                f.write(f'Mean Rank: {final_msg.get("mean_rank", "N/A")}\n')
                f.write(f'MAP Rank: {final_msg.get("map_rank", "N/A")}\n')
                f.write(f'Sqnorm Min: {final_msg.get("sqnorm_min", "N/A")}\n')
                f.write(f'Sqnorm Avg: {final_msg.get("sqnorm_avg", "N/A")}\n')
                f.write(f'Sqnorm Max: {final_msg.get("sqnorm_max", "N/A")}\n')
            elif opt.eval == 'hypernymy':
                f.write(f'Hypernymy Avg: {final_msg.get("eval_hypernymy_avg", "N/A")}\n')
                for key, val in final_msg.items():
                    if key.startswith('eval_') and key != 'eval_hypernymy_avg':
                        f.write(f'{key}: {val}\n')
        
        log.info(f'Evaluation results saved to {eval_file}')
